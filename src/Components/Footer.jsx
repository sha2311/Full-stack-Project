...
const SectionMenum = ({title , menus}) => {
  return (
    <div>
      <p className="font-bold border-b-1 mb-5 py-3 text-[#26395C] "> {title} </p>
      <div className="flex gap-16">
        { menus.map((menu, i)=>(
          <ul key={i}>
            { menu.items.map((item, idx)=>(
                <li className="text-[#656B89] leading-[27px] pb-3 cursor-pointer hover:text-[#0E8BFF] hover:scale-105 " key={idx} >
                  {item}
                </li>
            )) }
          </ul>
      ...
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-4 lg:gap-24 gap-2 ">
        {/* lrft side */}
          ...
          <div className="grid  grid-cols-1 lg:grid-cols-3  lg:gap-24 ms-auto col-span-3  ">
            ...
            <div className={cn(" w-full h-auto lg:h-[313px] bg-[#F5FAFF] p-4 rounded-[20px] ")}>
              
              <img className="w-fit pb-4" src={patnerSection.image} alt="Patner-Image" />
              <h3 className="text-[22px] font-bold leading-[27px] text-[#26395C] text-nowrap"> {patnerSection.title} </h3>
              <p className="text-[#656B89] py-4 leading-[27px] "> {patnerSection.description} </p>
